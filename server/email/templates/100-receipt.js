...
Linkki on voimassa 60 päivän ajan, jonka jälkeen tositetta ei ole enää mahdollista ladata. Säilytystä varten tallenna tiedosto itsellesi.
...
`
  ...
  runQueries() {
    ...
    return db.query('SELECT * FROM USERS WHERE id=$1', [this._user])
      ...
      .then(res => {
        if (res != null && res.rowCount > 0) {
          this._data.link = process.env.EMAIL_LINK_DOMAIN + '/api/receipt/' + res.rows[0].content
        }
      })
      .catch(err => {
        console.log(err)
